...
IMU_UPDATE_INTERVAL = 0.1  # How often to provide IMU updates over the socket
...
def get_imu(orientation=True, compass=False, gyroscope=False, accelerometer=False):
    ...
    if compass:
        data["compass"] = 360 - sense.get_compass()
    if gyroscope:
        data["gyroscope"] = sense.get_gyroscope()
    if accelerometer:
...
def background_thread(run_event, imu=True, enviro=False):
    ...
    try:
        while run_event.is_set():
            ...
            if imu:
                data = get_imu(orientation=True, compass=False,
...
@app.route('/compass')
def compass():
    data = get_imu(orientation=False, compass=True,
                   gyroscope=False, accelerometer=False)
    return jsonify(data)
